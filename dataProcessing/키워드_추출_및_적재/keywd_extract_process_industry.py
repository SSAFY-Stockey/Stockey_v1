...
input_file_path = "./inputs/real종목산업v3.csv"
...
print(f'산업 갯수 : {len(final_values)}개')
...
# industry_name = '식품 산업' -> '식품'
def find_by_industry_id_by_name(industryName):
    find_by_industry_id_by_name_sql = "SELECT industry_id FROM industry WHERE name LIKE CONCAT('%', %s, '%');"
    cursor.execute(find_by_industry_id_by_name_sql, (industryName,))
    return cursor.fetchone()
...
for file in fileList:
    ...
    sql = f"SELECT * FROM news where category = '{file}'" # 식품 산업
    ...
    for row in article_df.itertuples():
        ...
        for i in range(3):
            ...
            # news relation 채우기
            news_id = row.news_id # 뉴스 id
            if news_type == ECONOMY:
                exit(1)
                news_relation_insert(news_id, cur_keywd_id, None, None, ECONOMY)

            elif news_type == INDUSTRY:
                ind_category = row.category
                industry_name = ind_category.split(' ')[0]

                # 산업 id
                industry_id = find_by_industry_id_by_name(industry_name)
                news_relation_insert(news_id, cur_keywd_id, industry_id, None, INDUSTRY)
